...
// Strip matching surrounding double-quotes: "gamma_hat" -> gamma_hat.
// R write.csv quotes every header token and every string cell by default.
static void strip_quotes(std::string& s) {
    if (s.size() >= 2 && s.front() == '"' && s.back() == '"')
        s = s.substr(1, s.size() - 2);
}

// Split a line on commas; strips \r and surrounding quotes from each token.
...
int main(int argc, char* argv[]) {
    ...
    const double nu_tau       = 2.0,   s_tau        = 0.01;  // used iff pleiotropy
    const double nu_psi       = 3.0,   s_psi        = 0.01;
