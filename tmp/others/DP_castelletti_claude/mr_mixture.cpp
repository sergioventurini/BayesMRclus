...
// Read a CSV file (with a single header line) containing exactly 4 columns:
//   gamma_hat, Gamma_hat, sigma2X, sigma2Y
bool read_csv(const std::string& path,
    ...
    while (std::getline(f, line)) {
        ...
        if (row.size() < 4) {
            std::cerr << "Malformed row (need 4 columns): " << line << "\n";
            return false;
        }
        gamma_hat.push_back(row[0]);
        Gamma_hat.push_back(row[1]);
        sigma2X.push_back(row[2]);
        sigma2Y.push_back(row[3]);
    }
...
int main(int argc, char* argv[]) {
    ...
    if (argc < 3) {
        std::cerr << "Usage: " << argv[0] << " <data.csv> <S> [seed]\n"
                  << "  data.csv  CSV with header: gamma_hat,Gamma_hat,sigma2X,sigma2Y\n"
                  << "  S         number of MCMC iterations\n"
    ...
    const double nu_tau       = 4.0,   s_tau        = 1.0;  // used iff pleiotropy
    const double nu_psi       = 4.0,   s_psi        = 1.0;
    const double s2_betaprop  = 0.2,   s2_etaprop   = 0.2,  s2_deltaprop = 0.2;
    const int    m            = 2;
    const double a_alpha      = 1.0,   b_alpha      = 1.0;
