...
#' @export
bayesmr_het <- function(data, p = 1, G = 1, control = bayesmr_control(), prior = NULL, cl = NULL, post_all = FALSE) {
  if (p < 1)
  ...
  if (nchains > 1L && (have_mc || have_snow)) {
    bayesmr_het_fit_parallel <- function(c, data.c, p.c, G.c, control.c, prior.c, lib) {
      ...
      bayesmr_het_fit(data = data.c, p = p.c, G = G.c, control = control.c, prior = prior.c, start = start.c)
    }
    # environment(bayesmr_het_fit_parallel) <- .GlobalEnv # this prevents passing objects other than those needed for
    #                                                 # evaluating the bayesmr_het_fit_parallel function
    ...
    res <- if (have_mc) {
             ...
             parallel::mclapply(seq_len(nchains), bayesmr_het_fit_parallel, mc.cores = threads, mc.set.seed = TRUE,
             ...
             if (is.null(cl)) {
               ...
               res <- parallel::parLapply(cl, seq_len(nchains), bayesmr_het_fit_parallel, data.c = data, p.c = p, G.c = G, 
                 control.c = control, prior.c = prior, lib = .bayesmrEnv$path.to.me)
               parallel::stopCluster(cl)
               res
             } else parallel::parLapply(cl, seq_len(nchains), bayesmr_het_fit_parallel, data.c = data, p.c = p, G.c = G,
               control.c = control, prior.c = prior, lib = .bayesmrEnv$path.to.me)
           }
    ...
    for (ch in 1:nchains) {
      ...
      res[[ch]] <- bayesmr_het_fit(data = data, p = p, G = G, control = control, prior = prior, start = bayesmr_start)
