#' Estimate a BayesMR mixture model with random heterogeneity.
#'
#' `bayesmr_mix_het()` estimates a BayesMR mixture model with cluster-specific
#' causal effects and random heterogeneity. It initializes and runs one or more
#' MCMC chains, either serially or in parallel according to the settings supplied
#' through [`bayesmr_control()`].
#'
...
#'   as the number of iterations, burn-in, thinning, number of chains, seed,
#'   initial number of clusters, and parallelization options. Missing entries are
#'   filled with the defaults from [`bayesmr_control()`].
...
#'
#' @return An object of class [`bayesmr_mix_fit_list-class`] containing one
#'   fitted [`bayesmr_mix_het_fit-class`] object for each simulated chain.
#'
#' @details
...
#' [`bayesmr_mix_het_fit()`]. If `control$K_start` is `NULL` or smaller than 2,
#' it is set to 2 before initialization.
#'
...
#'
#' @seealso
#' [`bayesmr_mix_het_fit()`] for the lower-level single-chain fitting function;
#' [`bayesmr_data-class`] for the input data format;
#' [`bayesmr_mix_fit_list-class`] for the returned object;
