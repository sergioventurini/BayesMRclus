...
bayesmr_mix_het <- function(data, control = bayesmr_control(),
  ...
  if (nchains > 1L && (have_mc || have_snow)) {
    bayesmr_mix_het_fit_parallel <- function(c, data.c, control.c, prior.c, lib) {
      ...
      bayesmr_mix_het_fit(data = data.c, control = control.c, prior = prior.c, start = start.c)
    }
    # environment(bayesmr_mix_het_fit_parallel) <- .GlobalEnv # this prevents passing objects other than those needed for
    #                                                 # evaluating the bayesmr_mix_het_fit_parallel function
    ...
    res <- if (have_mc) {
             ...
             parallel::mclapply(seq_len(nchains), bayesmr_mix_het_fit_parallel, mc.cores = threads, mc.set.seed = TRUE,
             ...
             if (is.null(cl)) {
               ...
               res <- parallel::parLapply(cl, seq_len(nchains), bayesmr_mix_het_fit_parallel,
                 data.c = data, control.c = control, prior.c = prior,
                 lib = .bayesmrEnv$path.to.me)
               parallel::stopCluster(cl)
               res
             } else parallel::parLapply(cl, seq_len(nchains), bayesmr_mix_het_fit_parallel,
                data.c = data, control.c = control, prior.c = prior,
                lib = .bayesmrEnv$path.to.me)
           }
    ...
    for (ch in 1:nchains) {
      ...
      res[[ch]] <- bayesmr_mix_het_fit(data = data, control = control, prior = prior, start = bayesmr_start)
