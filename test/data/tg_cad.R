...
# extract data
tg <- extract_instruments(
  # outcomes = "ieu-a-302",      # GLGC triglycerides
  # outcomes = "ukb-b-9405",     # GLGC triglycerides
  outcomes = id_instruments,    # GLGC triglycerides
  p1 = 5e-08,
  clump = TRUE,
  r2 = 0.001,
  kb = 10000
)
cad  <- extract_outcome_data(
  snps = tg$SNP,
  outcomes = id_outcome,
  proxies = TRUE,
  rsq = 0.8
...
tg_cad <- dat_orig
save(tg_cad, file = "/Users/Sergio/dev/BayesMRclus/data/tg_cad.rda")
...
mrclust_res <- mrclust_results(tg_cad)
mrclust_res$p1
mrclust_res$tab
