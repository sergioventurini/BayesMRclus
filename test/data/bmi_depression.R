...
id_instruments <- "ieu-a-2"
id_outcome <- "ieu-a-1029"
...
depression  <- extract_outcome_data(
  snps = bmi$SNP,
  outcomes = id_outcome,
  proxies = TRUE,
  rsq = 0.8
...
dat_orig <- harmonise_data(
  exposure_dat = bmi,
  outcome_dat = depression,
  action = 2)

bmi_depression <- dat_orig
save(bmi_depression, file = "/Users/Sergio/dev/BayesMRclus/data/bmi_depression.rda")
...
mrclust_res <- mrclust_results(bmi_depression)
mrclust_res$p1
mrclust_res$tab
