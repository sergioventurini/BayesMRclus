...
dat <- data.frame(SNP = ldl_cad[, "SNP"],
                  beta_exposure = ldl_cad[, "beta.exposure"],
                  beta_outcome = ldl_cad[, "beta.outcome"],
                  se_exposure = ldl_cad[, "se.exposure"],
                  se_outcome = ldl_cad[, "se.outcome"])

n <- nrow(ldl_cad)
ldlcad_data <- new("bayesmr_data", data = dat, n = n, reorientation = TRUE)
data_tmp <- ldlcad_data@data
# summary(ldlcad_data)
# plot(ldlcad_data, se = TRUE)

# simulation setup
prm.prop <- list(beta = 1.25, psi = .025, tau = .025)
burnin <- 10000
nsim <- 20000
...
# MCMC simulation
system.time(res_BMR <- bayesmr_mix_het(ldlcad_data, control = control, prior = prior))
...
axis(1, at = seq(0, 1, length = p), lab = ldl_cad$SNP[ord], las = 2)
axis(2, at = seq(0, 1, length = p), lab = ldl_cad$SNP[ord], las = 2)

###

cols <- ifelse(touse == 1, "lightblue", "pink")
beta_post_ord <- t(beta_snps[ord, 1:S])
colnames(beta_post_ord) <- ldl_cad$SNP[ord]
par(mar = c(8, 5, 2, 2))
