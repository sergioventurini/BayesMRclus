...
dat <- data.frame(SNP = tg_insulin[, "SNP"],
                  beta_exposure = tg_insulin[, "beta.exposure"],
                  beta_outcome = tg_insulin[, "beta.outcome"],
                  se_exposure = tg_insulin[, "se.exposure"],
                  se_outcome = tg_insulin[, "se.outcome"])

n <- nrow(tg_insulin)
tginsulin_data <- new("bayesmr_data", data = dat, reorientation = TRUE)
...
# MCMC simulation
system.time(res_BMR <- bayesmr_mix_het(tginsulin_data, control = control, prior = prior))
...
axis(1, at = seq(0, 1, length = p), lab = tg_insulin$SNP[ord], las = 2)
axis(2, at = seq(0, 1, length = p), lab = tg_insulin$SNP[ord], las = 2)

###

cols <- ifelse(touse == 1, "lightblue", "pink")
beta_post_ord <- t(beta_snps[ord, 1:S])
colnames(beta_post_ord) <- tg_insulin$SNP[ord]
par(mar = c(8, 5, 2, 2))
...
# model selection
modsel <- MRPATH_selectModel(tg_insulin, K_range = 1:3, Nreps = 20,
                             altModel = FALSE, verbose = TRUE)

K <- 2

# initial values
initVals <- MRPATH_optimizeInitVals(K, tg_insulin, Nreps = 10, verbose = FALSE,
                                    altModel = FALSE, init_seed = 3110)
# initVals <- list("m_X" = mean(tg_insulin$beta.exposure),
#                 "lambdaX" = sd(tg_insulin$beta.exposure),
#                 "pis" = rep(1/K, K),
#                 "mus" = c(-.9, .4),
#                 "sds" = c(.9, .4))

### Run MC-EM algorithm
MCEM_fit <- MR_PATH(K, tg_insulin, initVals$initVals)

### Compute SNP-specific cluster membership probabilities 
clustermemb_prob <- computeClusterMembProb(tg_insulin, MCEM_fit = MCEM_fit)
MRPATH_barplot(tg_insulin, MCEM_fit, ret.snps = TRUE)
