...
n <- nrow(dat)
hdl_chd <- new("bayesmr_data", data = dat, reorientation = TRUE)
...
alpha_tau <- median(dat_tmp[, "se_outcome"])
prior <- bayesmr_prior(psi = list(alpha = alpha_psi, nu = 3),
                       tau = list(alpha = alpha_tau, nu = 3),
                       gamma = list(mean = 0, var = 10),
                       beta = list(mean = 0, var = 10),
                       alpha = list(a = 2, b = 1))

# MCMC simulation
res_BNPM_PMR <- bayesmr_mix_het(hdl_chd, control = control, prior = prior)
...
results <- run_bnpmr_postprocess(
  ...
  fig_dir   = "/Users/Sergio/dev/BayesMRclus/test/examples_mix_het/example_hdl_chd/figures/PMR",  # NULL to suppress figure output
  ci        = 0.95,
  ordering  = "post_mean",
  loss      = "binder",
  verbose   = FALSE,
...
# write.csv(mcmc_sim, file = "/Users/Sergio/dev/BayesMRclus/test/examples_mix_het/example_hdl_chd/output/PMR/mcmc_sim.csv")
# write.csv(results$K_posterior , file = "/Users/Sergio/dev/BayesMRclus/test/examples_mix_het/example_hdl_chd/output/PMR/Kpost.csv")
# write.csv(results$binder$partition, file = "/Users/Sergio/dev/BayesMRclus/test/examples_mix_het/example_hdl_chd/output/PMR/binder.csv")
# write.csv(results$clusters, file = "/Users/Sergio/dev/BayesMRclus/test/examples_mix_het/example_hdl_chd/output/PMR/clus.csv")
# write.csv(results$bma, file = "/Users/Sergio/dev/BayesMRclus/test/examples_mix_het/example_hdl_chd/output/PMR/bma.csv")
...
cl_agr_res <- compare_partitions(
  ...
  fig_dir                = "/Users/Sergio/dev/BayesMRclus/test/examples_mix_het/example_hdl_chd/figures/PMR",
  csv_dir                = "/Users/Sergio/dev/BayesMRclus/test/examples_mix_het/example_hdl_chd/output/PMR",
  verbose                = FALSE
)
...
save.image(file = "/Users/Sergio/dev/BayesMRclus/test/examples_mix_het/example_hdl_chd/output/PMR/out_PMR.rda")
