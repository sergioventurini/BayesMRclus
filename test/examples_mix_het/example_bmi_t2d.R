...
dat <- data.frame(SNP = bmi_t2d[, "SNP"],
                  beta_exposure = bmi_t2d[, "beta.exposure"],
                  beta_outcome = bmi_t2d[, "beta.outcome"],
                  se_exposure = bmi_t2d[, "se.exposure"],
                  se_outcome = bmi_t2d[, "se.outcome"])

n <- nrow(bmi_t2d)
bmit2d_data <- new("bayesmr_data", data = dat, reorientation = TRUE)
data_tmp <- bmit2d_data@data
# summary(bmit2d_data)
# plot(bmit2d_data, se = TRUE)

# simulation setup
prm.prop <- list(beta = 2, psi = .18, tau = .18)
burnin <- 10000
nsim <- 20000
...
# MCMC simulation
system.time(res_BMR <- bayesmr_mix_het(bmit2d_data, control = control, prior = prior))
...
axis(1, at = seq(0, 1, length = p), lab = bmi_t2d$SNP[ord], las = 2)
axis(2, at = seq(0, 1, length = p), lab = bmi_t2d$SNP[ord], las = 2)

###

cols <- ifelse(touse == 1, "lightblue", "pink")
beta_post_ord <- t(beta_snps[ord, 1:S])
colnames(beta_post_ord) <- bmi_t2d$SNP[ord]
par(mar = c(8, 5, 2, 2))
