...
# prepare data
data("sbp_cad", package = "BayesMRclus")

n <- nrow(sbp_cad)
mrclust_sbpcad <- new("bayesmr_data", data = sbp_cad, n = n, reorientation = TRUE)
data_tmp <- mrclust_sbpcad@data
# summary(mrclust_sbpcad)
# plot(mrclust_sbpcad, se = TRUE)

# simulation setup
prm.prop <- list(beta = sqrt(.01), psi = sqrt(.1), tau = sqrt(1))
burnin <- 0
nsim <- 10000
...
alpha_psi <- median(data_tmp[, "se_exposure"]) #0.01
alpha_tau <- median(data_tmp[, "se_outcome"])
prior <- bayesmr_prior(psi = list(alpha = alpha_psi, nu = 3),
                       tau = list(alpha = alpha_tau, nu = 3),
                       gamma = list(mean = 0, var = 1e1),
                       beta = list(mean = 0, var = .5),
                       alpha = list(a = 3, b = 1))

# MCMC simulation
system.time(res_BMR <- bayesmr_mix_het(mrclust_sbpcad, control = control, prior = prior))
...
summary(res_BMR@results[[1]]@gamma.chain)
summary(res_BMR@results[[1]]@alpha.chain)
summary(res_BMR@results[[1]]@psi.chain)
summary(res_BMR@results[[1]]@tau.chain)
plot(res_BMR@results[[1]]@gamma.chain, type = "l")
plot(res_BMR@results[[1]]@alpha.chain, type = "l")
plot(res_BMR@results[[1]]@psi.chain, type = "l")
acf(res_BMR@results[[1]]@psi.chain)
plot(res_BMR@results[[1]]@tau.chain, type = "l")
acf(res_BMR@results[[1]]@tau.chain)
...
cbind(cluster_mean = tapply(means, bl, mean), cluster_size = table(touse))
...
axis(1, at = seq(0, 1, length = p), lab = sbp_cad$SNP[ord], las = 2)
axis(2, at = seq(0, 1, length = p), lab = sbp_cad$SNP[ord], las = 2)

###

cols <- ifelse(touse == 1, "lightblue", "pink")
beta_post_ord <- t(beta_snps[ord, 1:S])
colnames(beta_post_ord) <- sbp_cad$SNP[ord]
par(mar = c(8, 5, 2, 2))
