...
# prepare data
data("pp_cad", package = "BayesMRclus")

n <- nrow(pp_cad)
mrclust_ppcad <- new("bayesmr_data", data = pp_cad, n = n, reorientation = TRUE)
data_tmp <- mrclust_ppcad@data
# summary(mrclust_ppcad)
# plot(mrclust_ppcad, se = TRUE)
...
# MCMC simulation
system.time(res_BMR <- bayesmr_mix_het(mrclust_ppcad, control = control, prior = prior))
...
axis(1, at = seq(0, 1, length = p), lab = pp_cad$SNP[ord], las = 2)
axis(2, at = seq(0, 1, length = p), lab = pp_cad$SNP[ord], las = 2)

###

cols <- ifelse(touse == 1, "lightblue", "pink")
beta_post_ord <- t(beta_snps[ord, 1:S])
colnames(beta_post_ord) <- pp_cad$SNP[ord]
par(mar = c(8, 5, 2, 2))
