...
# prepare data
data("dbp_cad", package = "BayesMRclus")

n <- nrow(dbp_cad)
mrclust_dbpcad <- new("bayesmr_data", data = dbp_cad, n = n, reorientation = TRUE)
data_tmp <- mrclust_dbpcad@data
# summary(mrclust_dbpcad)
# plot(mrclust_dbpcad, se = TRUE)

# simulation setup
prm.prop <- list(beta = sqrt(.01), psi = sqrt(.15), tau = sqrt(.5))
burnin <- 0
nsim <- 10000
...
alpha_psi <- 0.01
alpha_tau <- 2*median(data_tmp[, "se_outcome"])
...
# MCMC simulation
system.time(res_BMR <- bayesmr_mix_het(mrclust_dbpcad, control = control, prior = prior))
...
axis(1, at = seq(0, 1, length = p), lab = dbp_cad$SNP[ord], las = 2)
axis(2, at = seq(0, 1, length = p), lab = dbp_cad$SNP[ord], las = 2)

###

cols <- ifelse(touse == 1, "lightblue", "pink")
beta_post_ord <- t(beta_snps[ord, 1:S])
colnames(beta_post_ord) <- dbp_cad$SNP[ord]
par(mar = c(8, 5, 2, 2))
