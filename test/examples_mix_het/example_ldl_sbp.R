...
# prepare data
data("ldl_sbp", package = "BayesMRclus")

dat <- data.frame(SNP = ldl_sbp[, "SNP"],
                  beta_exposure = ldl_sbp[, "beta.exposure"],
                  beta_outcome = ldl_sbp[, "beta.outcome"],
                  se_exposure = ldl_sbp[, "se.exposure"],
                  se_outcome = ldl_sbp[, "se.outcome"])

n <- nrow(ldl_sbp)
ldlsbp_data <- new("bayesmr_data", data = dat, reorientation = TRUE)
data_tmp <- ldlsbp_data@data
# summary(ldlsbp_data)
# plot(ldlsbp_data, se = TRUE)

# simulation setup
prm.prop <- list(beta = 3, psi = .075, tau = .075)
burnin <- 10000
nsim <- 20000
...
# MCMC simulation
system.time(res_BMR <- bayesmr_mix_het(ldlsbp_data, control = control, prior = prior))
...
axis(1, at = seq(0, 1, length = p), lab = ldl_sbp$SNP[ord], las = 2)
axis(2, at = seq(0, 1, length = p), lab = ldl_sbp$SNP[ord], las = 2)

###

cols <- ifelse(touse == 1, "lightblue", "pink")
beta_post_ord <- t(beta_snps[ord, 1:S])
colnames(beta_post_ord) <- ldl_sbp$SNP[ord]
par(mar = c(8, 5, 2, 2))
