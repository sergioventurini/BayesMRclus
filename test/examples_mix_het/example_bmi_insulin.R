...
# prepare data
data("bmi_insulin", package = "BayesMRclus")

dat <- data.frame(SNP = bmi_insulin[, "SNP"],
                  beta_exposure = bmi_insulin[, "beta.exposure"],
                  beta_outcome = bmi_insulin[, "beta.outcome"],
                  se_exposure = bmi_insulin[, "se.exposure"],
                  se_outcome = bmi_insulin[, "se.outcome"])

n <- nrow(bmi_insulin)
bmiinsulin_data <- new("bayesmr_data", data = dat, n = n, harmonization = TRUE)
data_tmp <- bmiinsulin_data@data
# summary(bmiinsulin_data)
# plot(bmiinsulin_data, se = TRUE)

# simulation setup
prm.prop <- list(beta = .3, psi = .25, tau = .25)
burnin <- 20000
nsim <- 40000
...
control <- list(burnin = burnin, nsim = nsim, beta.prop = prm.prop[["beta"]],
                ...
                beta.m = 2, nchains = nchains, thin = 10,
...
prior <- bayesmr_prior(psi = list(alpha = alpha_psi, nu = 3),
                       ...
                       beta = list(mean = 0, var = .25),
                       alpha = list(a = 3, b = 1))

# MCMC simulation
system.time(res_BMR <- bayesmr_mix_het(bmiinsulin_data, control = control, prior = prior))
...
vi <- salso(x = res_BMR@results[[1]]@xi.chain, loss = "VI", maxNClusters = 4)
table(vi)
bl <- salso(x = res_BMR@results[[1]]@xi.chain, loss = "binder", maxNClusters = 4)
table(bl)
touse <- bl
...
axis(1, at = seq(0, 1, length = p), lab = bmi_insulin$SNP[ord], las = 2)
axis(2, at = seq(0, 1, length = p), lab = bmi_insulin$SNP[ord], las = 2)

###

cols <- ifelse(touse == 1, "lightblue", "pink")
beta_post_ord <- t(beta_snps[ord, 1:S])
colnames(beta_post_ord) <- bmi_insulin$SNP[ord]
par(mar = c(8, 5, 2, 2))
...
# model selection
modsel <- MRPATH_selectModel(bmi_insulin, K_range = 1:3, Nreps = 20,
                             altModel = FALSE, verbose = TRUE)

K <- 2

# initial values
initVals <- MRPATH_optimizeInitVals(K, bmi_insulin, Nreps = 10, verbose = FALSE,
                                    altModel = FALSE, init_seed = 3110)
# initVals <- list("m_X" = mean(bmi_insulin$beta.exposure),
#                 "lambdaX" = sd(bmi_insulin$beta.exposure),
#                 "pis" = rep(1/K, K),
#                 "mus" = c(-.9, .4),
#                 "sds" = c(.9, .4))

### Run MC-EM algorithm
MCEM_fit <- MR_PATH(K, bmi_insulin, initVals$initVals)

### Compute SNP-specific cluster membership probabilities 
clustermemb_prob <- computeClusterMembProb(bmi_insulin, MCEM_fit = MCEM_fit)
MRPATH_barplot(bmi_insulin, MCEM_fit, ret.snps = FALSE)
