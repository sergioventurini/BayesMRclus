...
whradjbmi_insulin <- new("bayesmr_data", data = dat, reorientation = TRUE)
...
# write.csv(results$clusters_level, file = "/Users/Sergio/dev/BayesMRclus/test/examples_mix_het/example_whradjbmi_insulin/output/PMR/clus.csv")
...
if (!exists("mrclust_res")) {
  ...
  # mrclust_res$p1
  mrclust_res$tab
  mrclust_res$cluster$cluster_class
}
