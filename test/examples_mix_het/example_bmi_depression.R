...
# prepare data
data("bmi_depression", package = "BayesMRclus")

dat <- data.frame(SNP = bmi_depression[, "SNP"],
                  beta_exposure = bmi_depression[, "beta.exposure"],
                  beta_outcome = bmi_depression[, "beta.outcome"],
                  se_exposure = bmi_depression[, "se.exposure"],
                  se_outcome = bmi_depression[, "se.outcome"])

n <- nrow(bmi_depression)
bmidep_data <- new("bayesmr_data", data = dat, reorientation = TRUE)
data_tmp <- bmidep_data@data
# summary(bmidep_data)
# plot(bmidep_data, se = TRUE)

# simulation setup
prm.prop <- list(beta = 1.5, psi = .25, tau = .25)
burnin <- 10000
nsim <- 20000
...
# MCMC simulation
system.time(res_BMR <- bayesmr_mix_het(bmidep_data, control = control, prior = prior))
...
axis(1, at = seq(0, 1, length = p), lab = bmi_depression$SNP[ord], las = 2)
axis(2, at = seq(0, 1, length = p), lab = bmi_depression$SNP[ord], las = 2)

###

cols <- ifelse(touse == 1, "lightblue", "pink")
beta_post_ord <- t(beta_snps[ord, 1:S])
colnames(beta_post_ord) <- bmi_depression$SNP[ord]
par(mar = c(8, 5, 2, 2))
