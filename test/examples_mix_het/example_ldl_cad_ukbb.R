...
# prepare data
data("ldl2_cad", package = "BayesMRclus")

dat <- data.frame(SNP = ldl2_cad[, "SNP"],
                  beta_exposure = ldl2_cad[, "beta.exposure"],
                  beta_outcome = ldl2_cad[, "beta.outcome"],
                  se_exposure = ldl2_cad[, "se.exposure"],
                  se_outcome = ldl2_cad[, "se.outcome"])

n <- nrow(ldl2_cad)
ldl2cad_data <- new("bayesmr_data", data = dat, n = n, harmonization = TRUE)
data_tmp <- ldl2cad_data@data
# summary(ldl2cad_data)
# plot(ldl2cad_data, se = TRUE)

# simulation setup
prm.prop <- list(beta = 2.5, psi = .025, tau = .025)
burnin <- 10000
nsim <- 20000
...
# MCMC simulation
system.time(res_BMR <- bayesmr_mix_het(ldl2cad_data, control = control, prior = prior))
...
axis(1, at = seq(0, 1, length = p), lab = ldl2_cad$SNP[ord], las = 2)
axis(2, at = seq(0, 1, length = p), lab = ldl2_cad$SNP[ord], las = 2)

###

cols <- ifelse(touse == 1, "lightblue", "pink")
beta_post_ord <- t(beta_snps[ord, 1:S])
colnames(beta_post_ord) <- ldl2_cad$SNP[ord]
par(mar = c(8, 5, 2, 2))
