...
# prepare data
data("bmi_cad", package = "BayesMRclus")

dat <- data.frame(SNP = bmi_cad[, "SNP"],
                  beta_exposure = bmi_cad[, "beta.exposure"],
                  beta_outcome = bmi_cad[, "beta.outcome"],
                  se_exposure = bmi_cad[, "se.exposure"],
                  se_outcome = bmi_cad[, "se.outcome"])

n <- nrow(bmi_cad)
bmicad_data <- new("bayesmr_data", data = dat, n = n, reorientation = TRUE)
data_tmp <- bmicad_data@data
# summary(bmicad_data)
# plot(bmicad_data, se = TRUE)

# simulation setup
prm.prop <- list(beta = 1.0, psi = .12, tau = .12)
burnin <- 10000
nsim <- 20000
...
# MCMC simulation
system.time(res_BMR <- bayesmr_mix_het(bmicad_data, control = control, prior = prior))
...
axis(1, at = seq(0, 1, length = p), lab = bmi_cad$SNP[ord], las = 2)
axis(2, at = seq(0, 1, length = p), lab = bmi_cad$SNP[ord], las = 2)

###

cols <- ifelse(touse == 1, "lightblue", "pink")
beta_post_ord <- t(beta_snps[ord, 1:S])
colnames(beta_post_ord) <- bmi_cad$SNP[ord]
par(mar = c(8, 5, 2, 2))
