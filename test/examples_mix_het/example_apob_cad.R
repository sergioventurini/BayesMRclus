...
# prepare data
data("apob_cad", package = "BayesMRclus")

dat <- data.frame(SNP = apob_cad[, "SNP"],
                  beta_exposure = apob_cad[, "beta.exposure"],
                  beta_outcome = apob_cad[, "beta.outcome"],
                  se_exposure = apob_cad[, "se.exposure"],
                  se_outcome = apob_cad[, "se.outcome"])

n <- nrow(apob_cad)
apob_data <- new("bayesmr_data", data = dat, reorientation = TRUE)
data_tmp <- apob_data@data
# summary(apob_data)
# plot(apob_data, se = TRUE)

# simulation setup
prm.prop <- list(beta = .5, psi = .075, tau = .075)
burnin <- 10000
nsim <- 20000
...
# MCMC simulation
system.time(res_BMR <- bayesmr_mix_het(apob_data, control = control, prior = prior))
...
axis(1, at = seq(0, 1, length = p), lab = apob_cad$SNP[ord], las = 2)
axis(2, at = seq(0, 1, length = p), lab = apob_cad$SNP[ord], las = 2)

###

cols <- ifelse(touse == 1, "lightblue", "pink")
beta_post_ord <- t(beta_snps[ord, 1:S])
colnames(beta_post_ord) <- apob_cad$SNP[ord]
par(mar = c(8, 5, 2, 2))
