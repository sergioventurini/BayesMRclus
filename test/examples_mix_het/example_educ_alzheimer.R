...
dat <- data.frame(SNP = educ_alzheimer[, "SNP"],
                  beta_exposure = educ_alzheimer[, "beta.exposure"],
                  beta_outcome = educ_alzheimer[, "beta.outcome"],
                  se_exposure = educ_alzheimer[, "se.exposure"],
                  se_outcome = educ_alzheimer[, "se.outcome"])

n <- nrow(educ_alzheimer)
educalz_data <- new("bayesmr_data", data = dat, reorientation = TRUE)
data_tmp <- educalz_data@data
# summary(educalz_data)
# plot(educalz_data, se = TRUE)

# simulation setup
prm.prop <- list(beta = 2, psi = .15, tau = .15)
burnin <- 10000
nsim <- 20000
...
# MCMC simulation
system.time(res_BMR <- bayesmr_mix_het(educalz_data, control = control, prior = prior))
...
axis(1, at = seq(0, 1, length = p), lab = educ_alzheimer$SNP[ord], las = 2)
axis(2, at = seq(0, 1, length = p), lab = educ_alzheimer$SNP[ord], las = 2)

###

cols <- ifelse(touse == 1, "lightblue", "pink")
beta_post_ord <- t(beta_snps[ord, 1:S])
colnames(beta_post_ord) <- educ_alzheimer$SNP[ord]
par(mar = c(8, 5, 2, 2))
