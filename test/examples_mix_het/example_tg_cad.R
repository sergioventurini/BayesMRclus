...
# prepare data
data("tg_cad", package = "BayesMRclus")

dat <- data.frame(SNP = tg_cad[, "SNP"],
                  beta_exposure = tg_cad[, "beta.exposure"],
                  beta_outcome = tg_cad[, "beta.outcome"],
                  se_exposure = tg_cad[, "se.exposure"],
                  se_outcome = tg_cad[, "se.outcome"])

n <- nrow(tg_cad)
tgcad_data <- new("bayesmr_data", data = tg_cad, reorientation = TRUE)
data_tmp <- tgcad_data@dat
# summary(tgcad_data)
# plot(tgcad_data, se = TRUE)

# simulation setup
prm.prop <- list(beta = 2, psi = .1, tau = .1)
burnin <- 10000
nsim <- 20000
...
# MCMC simulation
system.time(res_BMR <- bayesmr_mix_het(tgcad_data, control = control, prior = prior))
...
axis(1, at = seq(0, 1, length = p), lab = tg_cad$SNP[ord], las = 2)
axis(2, at = seq(0, 1, length = p), lab = tg_cad$SNP[ord], las = 2)

###

cols <- ifelse(touse == 1, "lightblue", "pink")
beta_post_ord <- t(beta_snps[ord, 1:S])
colnames(beta_post_ord) <- tg_cad$SNP[ord]
par(mar = c(8, 5, 2, 2))
