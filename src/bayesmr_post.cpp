...
// full conditional distribution of the psi parameter
void logpost_psi(double* lpost, const double psi,
  ...
  // this is needed because dhalft() is vectorized and there is no implicit
  // conversion in C++ between std::vector<double> and double
  const double logprior = dhalft(
    std::vector<double>{psi},
    std::vector<double>{alpha_psi},
    std::vector<double>{nu_psi},
    true )[0];
...
// full conditional distribution of the tau parameter
