...
void bayesmr_mcmc_noclus(
  ...
  for (int iter = 1; iter <= totiter; ++iter) {
    ...
    if ((iter % 500) == 0 && verbose) {
      double accept_rate_beta = accept_beta / static_cast<double>(G*iter);
      REprintf("   iteration %d/%d ==> acceptance beta: %1.4f\n", iter, totiter, accept_rate_beta);
    }
...
void bayesmr_mcmc_noclus_ranhet(
  double* gamma_chain,
  double* beta_chain,
  double* psi_chain,
  double* tau_chain,
  ...
  const double rhyper_beta_var,
  const double sigma2_beta,                   // beta proposal variance (note sqrt used)
  const double C_psi,                         // psi2 proposal parameter
  const double C_tau,                         // tau2 proposal parameter
  int totiter,
  int n,
  int p,
  int G,
  ...
  double psi2_old = psi_old * psi_old;
  double accept_psi = 0.0;   // track total accept for psi2 chain
  double tau_old = tau_p;
  double tau2_old = tau_old * tau_old;
  double accept_tau = 0.0;   // track total accept for tau2 chain
  accept[0] = 0.0;
  accept[1] = 0.0;
  accept[2] = 0.0;
  ...
  for (int iter = 1; iter <= totiter; ++iter) {
    for (int i = 0; i < n; ++i) {
      psi2_j[i] = sigma2_X[i] + psi2_old;
      tau2_j[i] = sigma2_Y[i] + tau2_old;
    }
    ...
    bool invalid_v = false;
    for (int i = 0; i < n; ++i) {
      a_j[i] = beta2 * psi2_old + tau2_j[i];
      const double c = beta_old * psi2_old;
      const double c2 = c * c;
      v_j[i] = a_j[i] * psi2_j[i] - c2;
    ...
    logpost_beta(&lpost_prop, beta_prop, gamma_new,
      rhyper_beta_mean, rhyper_beta_var, psi2_old, tau2_old, n,
      gammahat_j.data(), Gammahat_j.data(), sigma2_X.data(), sigma2_Y.data());
    ...
    accept[0] += accepted_beta;

    // store beta
    beta_chain[iter - 1] = beta_old;

    // --------------------------------------
    // 3) Metropolis-Hastings update for psi2
    // --------------------------------------
    // Propose new psi2 (random-walk Uniform)
    double psi_prop = R::runif(std::max(0.0, psi_old - C_psi), psi_old + C_psi);

    // compute log-posterior for proposed and current betas
    lpost_prop = 0.0;
    lpost_old = 0.0;

    logpost_psi(&lpost_prop, psi_prop, beta_old, gamma_new, tau_old,
      rhyper_alpha_psi, rhyper_nu_psi, n,
      gammahat_j.data(), Gammahat_j.data(), sigma2_X.data(), sigma2_Y.data());

    logpost_psi(&lpost_old, psi_old, beta_old, gamma_new, tau_old,
      rhyper_alpha_psi, rhyper_nu_psi, n,
      gammahat_j.data(), Gammahat_j.data(), sigma2_X.data(), sigma2_Y.data());
    ...
    u = R::runif(0.0, 1.0);
    int accepted_psi = (u < prob) ? 1 : 0;
    if (accepted_psi) {
      psi_old = psi_prop;
      accept_psi += 1.0;
    }
    accept[1] += accepted_psi;

    psi2_old = psi_old * psi_old;

    // store psi
    psi_chain[iter - 1] = psi_old;

    // --------------------------------------
    // 4) Metropolis-Hastings update for tau2
    // --------------------------------------
    // Propose new tau2 (random-walk Uniform)
    double tau_prop = R::runif(std::max(0.0, tau_old - C_tau), tau_old + C_tau);

    // compute log-posterior for proposed and current betas
    lpost_prop = 0.0;
    lpost_old = 0.0;

    logpost_tau(&lpost_prop, tau_prop, beta_old, gamma_new, psi_old,
      rhyper_alpha_tau, rhyper_nu_tau, n,
      gammahat_j.data(), Gammahat_j.data(), sigma2_X.data(), sigma2_Y.data());

    logpost_tau(&lpost_old, tau_old, beta_old, gamma_new, psi_old,
      rhyper_alpha_tau, rhyper_nu_tau, n,
      gammahat_j.data(), Gammahat_j.data(), sigma2_X.data(), sigma2_Y.data());
    ...
    u = R::runif(0.0, 1.0);
    int accepted_tau = (u < prob) ? 1 : 0;
    if (accepted_tau) {
      tau_old = tau_prop;
      accept_tau += 1.0;
    }
    accept[1] += accepted_tau;

    tau2_old = tau_old * tau_old;
    ...
    const double logprior_gamma = R::dnorm(gamma_new, rhyper_gamma_mean, sqrt_rhyper_gamma_var, 1);
    const double logprior_beta  = R::dnorm(beta_old, rhyper_beta_mean, sqrt_rhyper_beta_var, 1);
    const double logprior_psi2  = dhalft(
      std::vector<double>{psi_old},
      std::vector<double>{rhyper_alpha_psi},
      std::vector<double>{rhyper_nu_psi},
      true )[0];
    const double logprior_tau2  = dhalft(
      std::vector<double>{tau_old},
      std::vector<double>{rhyper_alpha_tau},
      std::vector<double>{rhyper_nu_tau},
      true )[0];
    logprior[iter - 1] = logprior_gamma + logprior_beta + logprior_psi2 + logprior_tau2;
    ...
    if ((iter % 500) == 0 && verbose) {
      double accept_rate_beta = accept_beta / static_cast<double>(G*iter);
      double accept_rate_psi2 = accept_psi / static_cast<double>(G*iter);
      double accept_rate_tau2 = accept_tau / static_cast<double>(G*iter);
      REprintf("   iteration %d/%d ==> acceptance beta: %1.4f\n", iter, totiter, accept_rate_beta);
      REprintf("   iteration %d/%d ==> acceptance psi2: %1.4f\n", iter, totiter, accept_rate_psi2);
      REprintf("   iteration %d/%d ==> acceptance tau2: %1.4f\n", iter, totiter, accept_rate_tau2);
    }
